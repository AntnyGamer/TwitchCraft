...
internal sealed partial class TokenHandler(string path)
{
    ...
    public int GetBalance(string user)
        => TryGetBalance(user, out int balance) ? balance : 0;

    internal bool TryGetBalance(string user, out int balance)
    {
        balance = 0;
        ...
        lock (_gate)
        {
            if (!EnsureLoadedNoLock(normalized)) return false;
            _balances.TryGetValue(normalized, out balance);
            return true;
        }
    }
    ...
    internal int? TryTransfer(string from, string to, int amount, int maximumBalance)
    {
        lock (_gate)
        {
            ...
            if (newToBalance != toBalance) changes[to] = newToBalance;
            if (!SaveChangesNoLock(changes)) return null;
            SetCacheNoLock(from, fromBalance - amount);
            if (newToBalance != toBalance) SetCacheNoLock(to, newToBalance);
            return newToBalance - toBalance;
        }
    }
        ...
        int maximumBalance = 0)
    {
        ...
        lock (_gate)
        {
            ...
            {
                TwitchCraft_V1.ErrorHandling.LogNonFatal("Failed to save Twitch follow reward", ex);
                return FollowRewardResult.Failed;
            }
        }
    }
    ...
    public int AdjustBalances(IEnumerable<string> users, int delta, int maximumBalance = 0)
        => AdjustBalances(users, delta, maximumBalance, out _);

    internal int AdjustBalances(IEnumerable<string> users, int delta, int maximumBalance, out bool succeeded)
    {
        ...
        return AdjustDeltas(normalizedDeltas, maximumBalance, out succeeded);
    }

    public bool AdjustBalances(IEnumerable<KeyValuePair<string, int>> changes, int maximumBalance = 0)
    {
        ...
        _ = AdjustDeltas(normalizedDeltas, maximumBalance, out bool succeeded);
        return succeeded;
    }

    private int AdjustDeltas(Dictionary<string, int> normalizedDeltas, int maximumBalance, out bool succeeded)
    {
        succeeded = true;
        ...
        lock (_gate)
        {
            if (normalizedDeltas.Count == 1)
            {
                ...
                {
                    if (!EnsureLoadedNoLock(change.Key)) { succeeded = false; return 0; }
                    _balances.TryGetValue(change.Key, out int current);
                    int newBalance = ClampAdjusted(current, change.Value, maximumBalance);
                    if (newBalance == current) return 0;
                    succeeded = SaveChangedNoLock(change.Key, newBalance);
                    if (succeeded) SetCacheNoLock(change.Key, newBalance);
    ...
    public void CloseConnection()
    {
        lock (_gate)
        {
            ...
            {
                TwitchCraft_V1.ErrorHandling.LogNonFatal("Failed to close viewer token database", ex);
            }
            finally
            {
                _connection = null;
    ...
    public bool TryExportJson()
    {
        ...
        {
            TwitchCraft_V1.ErrorHandling.LogNonFatal("Failed to export readable viewer token JSON", ex);
            return false;
        }
    }
    ...
    {
        if (normalized.Length == 0 || balance <= 0)
        {
            _balances.Remove(normalized);
            return;
        }

        _balances[normalized] = balance;
    }
    ...
    private static string Normalize(string? user) => TwitchCraft_V1.CommandUserHelper.NormalizeUser(user);
