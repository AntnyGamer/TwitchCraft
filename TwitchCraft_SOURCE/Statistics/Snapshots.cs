...
{
    public StatisticsSnapshot GetSnapshot(CancellationToken cancellationToken = default)
    {
        ...
        while (true)
        {
            ...
            (string totalMostDangerousViewer, string totalNicestViewer) = StatisticsStore.GetTopViewers(streamerViewer);
            cancellationToken.ThrowIfCancellationRequested();
            lock (_statisticsGate)
            {
    ...
    {
        StatisticsStore.TryExportJson();
    }

    private StatisticsSnapshot BuildSnapshotNoLock(DateTime now)
    {
    ...
    private static TimeSpan? GetSessionSurvival(SessionStatistics session, DateTime now)
    {
        if (session.CurrentLifeWaitingForRespawn && !session.CurrentLifeHasStarted) return TimeSpan.MinValue;
    ...
    private static long GetLifeSeconds(SessionStatistics session, DateTime now)
    {
    ...
    private static bool TryExtractDeathScore(string message, out string playerName, out int deathScore)
    {
        ...
        {
            ReadOnlySpan<char> scoreText = message.AsSpan()[(hasIndex + hasMarker.Length)..objectiveIndex].Trim();
            if (MinecraftNameHelper.TryNormalizePlayerName(message.AsSpan(0, hasIndex), out string candidate) &&
        ...
        const string currentMissingPrefix = "Can't get value of tc_deaths for ";
        if (message.StartsWith(currentMissingPrefix, StringComparison.OrdinalIgnoreCase))
        {
            int playerEnd = message.IndexOf(';', currentMissingPrefix.Length);
            string candidate = (playerEnd < 0
                ? message[currentMissingPrefix.Length..]
                : message[currentMissingPrefix.Length..playerEnd]).Trim();
