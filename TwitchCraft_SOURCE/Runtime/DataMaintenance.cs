...
internal sealed class DataMaintenance(
    Func<TwitchCraftConfig?> getConfig,
...
{
    private readonly Func<TwitchCraftConfig?> _getConfig = getConfig ?? throw new ArgumentNullException(nameof(getConfig));
    ...
    internal async Task RunAsync(CancellationToken cancellationToken)
    {
        while (!cancellationToken.IsCancellationRequested)
        {
            try
            {
                ...
                if (optimizeHours > 0 &&
                    DateTime.UtcNow - _lastDatabaseOptimizeUtc >= TimeSpan.FromHours(optimizeHours) &&
                    _tokens.TryOptimize())
    ...
    internal static void PruneBackups(string root, int retentionCount)
    {
        ...
        {
            int byTimestamp = right.Timestamp.CompareTo(left.Timestamp);
            return byTimestamp != 0 ? byTimestamp : right.Directory.CreationTimeUtc.CompareTo(left.Directory.CreationTimeUtc);
        });
        ...
        out DateTime timestampUtc)
    {
        ...
        if ((name.Length != 15 &&
             (name.Length != 22 || name[15] != '-' || !int.TryParse(name.AsSpan(16), NumberStyles.HexNumber, CultureInfo.InvariantCulture, out _))) ||
