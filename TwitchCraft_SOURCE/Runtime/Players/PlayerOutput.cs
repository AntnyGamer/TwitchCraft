...
public sealed partial class MainHandler
{
    ...
    internal async Task ReadOutputAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            while (!cancellationToken.IsCancellationRequested)
            {
                ...
                {
                    _shellWindow?.AddServerLogLine(ErrorHandling.FormatLog("Server output line handling failed", ex));
                }
            }
        }
    ...
    private bool AddKnownPlayer(string playerName)
    {
        ...
        lock (_playerGate)
        {
            ...
            List<string> players = new(_knownPlayers.Count + 1);
            players.AddRange(_knownPlayers);
    ...
    private void CapturePlayers(string line)
    {
        ...
        if (!TryParseList(line, out List<string> players))
            return;
