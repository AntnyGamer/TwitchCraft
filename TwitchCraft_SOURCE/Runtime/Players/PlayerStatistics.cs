...
public sealed partial class MainHandler
{
    ...
    private void QueueDeathScore(string playerName)
    {
        ...
        RunSessionWork(
            async t =>
            {
                ...
                {
                    if (!string.Equals(
                            Volatile.Read(ref _deathScoreInitializedPlayerName),
    ...
    private static bool TryParseList(string response, out List<string> players)
    {
        players = [];
    ...
    {
        ReadOnlySpan<char> segment = value.AsSpan(0, length).Trim();
