...
public sealed partial class MainHandler
{
    ...
    public async Task<bool> SendServerCommandAsync(string command, CancellationToken cancellationToken, bool applyRemoteTimeout = true)
    {
        ...
        TwitchCraftConfig? activeConfig = _activeConfig;
    ...
    private async Task<bool> SendRCONCommandsAsync(TwitchCraftConfig config, List<string> commands, CancellationToken cancellationToken)
    {
        ...
        catch (RCONPartialResponseException ex)
        {
            if (cancellationToken.IsCancellationRequested)
                throw new OperationCanceledException(null, ex, cancellationToken);
            _shellWindow?.AddServerLogLine("At least one RCON command response was confirmed before the batch was interrupted.");
            return true;
        }
        catch (OperationCanceledException) when (!cancellationToken.IsCancellationRequested)
        {
    ...
    private Task<bool> WriteCommandNoLockAsync(Process process, string normalizedCommand, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            return WriteBytesNoLockAsync(process.StandardInput.BaseStream, rented, written + ServerCommandNewLineBytes.Length, cancellationToken);
        }
        catch { ArrayPool<byte>.Shared.Return(rented); throw; }
    }
    ...
    private async Task<bool> WriteBytesNoLockAsync(Stream baseStream, byte[] rented, int written, CancellationToken cancellationToken)
    {
        ...
        catch (Exception ex) when (writeCompleted && (ex is OperationCanceledException or ObjectDisposedException or InvalidOperationException))
        {
            return true;
        }
        finally
