...
public static partial class MinigameManager
{
    private static async Task RunGuessNumberAsync(MainHandler runtime, CancellationToken cancellationToken)
    {
        ...
        try
        {
            if (!TryStartGuessGame(runtime, out long roundID, out int minValue, out int maxValue))
                return;
    ...
    private static bool TryStartGuessGame(MainHandler runtime, out long roundID, out int minValue, out int maxValue)
    {
        roundID = 0;
        minValue = 1;
        maxValue = 100;
    ...
    private static void EvaluateGuess(MainHandler runtime, int guess, out bool active, out bool correct, out int targetNumber)
    {
        active = false;
        correct = false;
        targetNumber = 0;
