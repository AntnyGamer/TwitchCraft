...
internal static class TwitchOAuthAuthorizer
{
    ...
    internal const string ApplicationClientID = "291b194ilywj9gxpo919c2p9q08tdx";
    internal static bool IsOAuthConfigured => ApplicationClientID.Length > 0;
    ...
    public static async Task<TwitchOAuthResult> AuthorizeAsync(string clientID, CancellationToken cancellationToken)
    {
        ...
        {
            return Fail("Could not open the Twitch authorization page. " + ex.Message);
        }

        TimeSpan pollDelay = TimeSpan.FromSeconds(Math.Max(device.IntervalSeconds, 1));
        ...
        using CancellationTokenSource authorizationCts = CancellationTokenSource.CreateLinkedTokenSource(cancellationToken);
        authorizationCts.CancelAfter(allowedWait);
        try
        {
            while (true)
            {
                await Task.Delay(pollDelay, authorizationCts.Token).ConfigureAwait(false);
                DeviceTokenPollResult poll = await PollForTokenAsync(
                    normalizedClientID,
                    device.DeviceCode,
                    authorizationCts.Token).ConfigureAwait(false);
        ...
        out string error)
    {
        ...
        login = TwitchCraft_V1.CommandUserHelper.NormalizeUser(GetString(root, "login"));
