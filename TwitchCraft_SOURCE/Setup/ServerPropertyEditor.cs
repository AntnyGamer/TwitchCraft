...
public sealed class ServerPropertyEditor
{
    ...
    private static readonly HashSet<string> ManagedServerProperties = new(ManagedServerPropertyOrder, StringComparer.Ordinal) { "rcon.password" };

    public static string GetPropertiesPath(TwitchCraftConfig config)
    {
    ...
    public static string GetLevelName(TwitchCraftConfig config)
    {
        ...
        foreach (string line in File.ReadLines(propsPath, Encoding.UTF8))
        {
            ...
            if (equals >= 0 && MemoryExtensions.Equals(line.AsSpan(0, equals).Trim(), "level-name".AsSpan(), StringComparison.Ordinal))
    ...
    public static string GetWorldDirectory(TwitchCraftConfig config)
    {
    ...
    public static string NormalizeLevelName(string? levelName)
    {
        ...
        if (value == "." || value == ".." || Path.IsPathRooted(value) || value.IndexOfAny(Path.GetInvalidFileNameChars()) >= 0 ||
    ...
    public static void WriteInitialFiles(TwitchCraftConfig config)
    {
        ArgumentNullException.ThrowIfNull(config);
    ...
    public static void CleanupServerJars(string serverDirectory, string currentJarPath)
    {
        ...
        foreach (string path in Directory.EnumerateFiles(serverDirectory, "twitchcraft-server-*.jar"))
        {
            ...
            {
                TwitchCraft_V1.ErrorHandling.LogNonFatal("Failed to delete unused TwitchCraft-managed server jar", ex);
            }
        }
    }
    ...
    private static void AddProperty(Dictionary<string, string> props, string line)
    {
        ...
        props[key] = key.Equals("rcon.password", StringComparison.Ordinal)
