...
public sealed partial class ConfigurationStore
{
    ...
    private static void ResetStartMode(TwitchCraftConfig config)
    {
    ...
    private static void Normalize(TwitchCraftConfig config)
    {
        ...
        config.Twitch.BotToken = TwitchCraft_V1.TwitchTokenHelper.NormalizeAccessToken(config.Twitch.BotToken);
    ...
    private static void NormalizeEndpoint(ServerConfig server)
    {
        ...
        server.RemoteHost = TryNormalizeRemoteHost(host, out string normalizedHost) ? normalizedHost : string.Empty;
    }
