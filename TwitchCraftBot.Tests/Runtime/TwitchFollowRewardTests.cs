...
public sealed class TwitchFollowRewardTests
{
    [Fact]
    ...
    public void ParsesChannelFollowV2Notification()
    {
        ...
        Assert.True(BotMainHandler.TryParseFollowNotification(message, out BotMainHandler.FollowNotification notification));
        Assert.Equal("123456", notification.UserId);
        Assert.Equal("randomdudereincarnatedx3", notification.UserLogin);
        Assert.Equal(DateTimeOffset.Parse("2026-08-27T01:02:03.456Z"), notification.FollowedAt);
    }
    ...
    public void RejectsWrongSubscriptionOrMalformedFollow()
    {
        ...
        Assert.False(BotMainHandler.TryParseFollowNotification(wrongType, out _));
        Assert.False(BotMainHandler.TryParseFollowNotification(missingIdentity, out _));
    }
}
