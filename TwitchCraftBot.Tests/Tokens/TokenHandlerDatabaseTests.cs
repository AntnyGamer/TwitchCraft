...
public sealed class TokenHandlerDatabaseTests
{
    ...
    public void PositiveAwards_RespectMaximumBalanceWithoutBreakingSpending()
    {
        ...
        try
        {
            ...
            Assert.True(store.TrySpendNow("viewer", 30));
    ...
    public void FollowReward_ReportsActualAwardWhenMaximumBalanceIsReached()
    {
        ...
        {
            store.AdjustBalance("viewer", 90);
            FollowRewardResult result = store.TryRewardFollowerOnce(
                "123456",
                "viewer",
    ...
    public void FollowReward_IsPaidOnlyOncePerTwitchAccount()
    {
        ...
        try
        {
            Assert.Equal(
                ...
                store.TryRewardFollowerOnce("123456", "FirstName", DateTimeOffset.Parse("2026-08-27T01:02:03Z"), 50));
            Assert.Equal(
                FollowRewardResult.AlreadyRewarded,
                store.TryRewardFollowerOnce("123456", "RenamedUser", DateTimeOffset.Parse("2026-08-27T02:03:04Z"), 50));
    ...
    public void FollowReward_DeduplicationPersistsAcrossReopen()
    {
        ...
        try
        {
            Assert.Equal(
                FollowRewardResult.Rewarded,
                writer.TryRewardFollowerOnce("987654", "viewer", DateTimeOffset.UtcNow, 50));
        }
        finally
        {
            writer.CloseConnection();
        ...
        try
        {
            Assert.Equal(
                FollowRewardResult.AlreadyRewarded,
                reader.TryRewardFollowerOnce("987654", "viewer", DateTimeOffset.UtcNow, 50));
    ...
    public void FollowReward_RejectsInvalidIdentityWithoutChargingDatabase()
    {
        ...
        try
        {
            Assert.Equal(FollowRewardResult.Failed, store.TryRewardFollowerOnce("not-a-user-id", "viewer", DateTimeOffset.UtcNow, 50));
            Assert.Equal(FollowRewardResult.Failed, store.TryRewardFollowerOnce("123", "", DateTimeOffset.UtcNow, 50));
            Assert.Equal(FollowRewardResult.Failed, store.TryRewardFollowerOnce("123", "viewer", DateTimeOffset.UtcNow, 0));
            Assert.Equal(0, store.GetBalance("viewer"));
        }
        finally
        {
    ...
    public void BackupDatabase_CreatesAReadablePointInTimeCopy()
    {
        ...
        try
        {
            ...
            Assert.True(store.TryBackupDatabase(backupPath));
        ...
        try
        {
            ...
            Assert.True(backup.TryOptimizeDatabase());
        }
        finally
        {
