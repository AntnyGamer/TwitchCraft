...
public sealed class GameplayBehaviorIntegrationTests
{
    ...
    public async Task EffectCommand_MultipleEffectsChargesPerEffectAndDeliversEach()
    {
        ...
        Assert.Equal(3, commands.Count(command => command.StartsWith("effect give " + PlayerSelector + " minecraft:", StringComparison.Ordinal)));
        Assert.Equal(3, commands.Count(command => command.StartsWith("tellraw " + PlayerSelector + " ", StringComparison.Ordinal) &&
    ...
    public async Task EnchantCommand_RebuildsHeldItemAndChargesOnlyAfterDelivery()
    {
        ...
        string enchant = Assert.Single(commands.Where(command =>
            command.StartsWith("item replace entity " + PlayerSelector + " weapon.mainhand with minecraft:diamond_sword[", StringComparison.Ordinal)));
    ...
    public async Task RenameCommand_PreservesHeldItemAndAppliesRedeemerName()
    {
        ...
        string rename = Assert.Single(commands.Where(command =>
            command.StartsWith("item replace entity " + PlayerSelector + " weapon.mainhand with minecraft:diamond_sword[", StringComparison.Ordinal)));
    ...
    public async Task SwitchMilkCommand_UsesOneScopedTagAndAlwaysCleansItUp()
    {
        ...
        string first = Assert.Single(commands.Where(command => command.StartsWith("tag @a remove tc_switchmilk_", StringComparison.Ordinal)));
    ...
    public async Task ScaredCommand_SpawnsExactlyTwentyCatsWithWarningTitles()
    {
        ...
        Assert.Equal(1, commands.Count(command => command.StartsWith("title " + PlayerSelector + " times ", StringComparison.Ordinal)));
