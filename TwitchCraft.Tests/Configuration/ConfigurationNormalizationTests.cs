...
public sealed class ConfigurationNormalizationTests
{
    ...
    public void NormalizeRuntime_RepairsInvalidValuesWithoutChangingValidIdentityData()
    {
        TwitchCraftConfig config = new()
        {
            ...
            Identity = new IdentityConfig { StreamerMinecraftName = "  Player  " },
        ...
        Assert.Equal(2.5, heal.GlobalCooldownSeconds);
        Assert.True(config.Settings.CommandCustomizations.ContainsKey("heal"));
    }

    [Fact]
    public void NormalizeRuntime_OrdersAndPreservesCustomPassivePayoutRange()
    {
        TwitchCraftConfig config = new();
    ...
    {
        Assert.False(ConfigurationStore.TryNormalizeRCONPassword(" first\nsecond ", out string password));
        Assert.Equal("first\nsecond", password);
    }
    ...
    public void GenerateRCONPassword_ReturnsValidTwentyFourByteCredential()
    {
        ...
        Assert.True(ConfigurationStore.TryNormalizeRCONPassword(password, out string normalized));
        Assert.Equal(password, normalized);
        Assert.Equal(24, Convert.FromBase64String(password).Length);
    }
}
