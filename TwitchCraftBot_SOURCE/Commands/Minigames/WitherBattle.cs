...
{
    private static async Task RunWitherAsync(BotMainHandler runtime, CancellationToken cancellationToken)
    {
        if (!TryStartMinigame(runtime, "WitherBattle", out int runID))
            return;
        ...
        try
        {
            ...
            await PlaySoundAsync(runtime, "minecraft:entity.wither.shoot", cancellationToken).ConfigureAwait(false);
            await ShowSubtitleAsync(runtime, "WITHER BATTLE", cancellationToken).ConfigureAwait(false);
            ...
            lock (MinigameGate)
            {
                ...
                state = GetWitherStateNoLock(runtime);
                ...
                // StopLoopsCore/RefundWitherBets from refunding the same bets
